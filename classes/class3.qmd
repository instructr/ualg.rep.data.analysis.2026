...
# 4. Write a function that takes a numeric vector and returns
#    a named list with its mean, median, and sd.

# 5. Using base R or dplyr (your choice), compute the mean Sepal.Length
#    grouped by Species.

# 6. Make a scatter plot of Sepal.Length vs Petal.Length,
...
## Install RStudio Desktop {#sec-install-rstudio}
...
## Configure your Git identity {#sec-git-config}
...
# GitHub Account and SSH Authentication {#sec-github}
...
Open the `README.md` file and add a few sentences describing what the project is about. You will expand this as the project develops.

## Update .gitignore {#sec-gitignore}

Open `.gitignore` and add entries for things that should never be version controled:
...
2. Click **Commit**.
3. Write a commit message: `First commit`.
4. Click **Commit**.
...
| Errors on paths with spaces (Windows) | Project cloned into a path like `C:\Users\Ana Sofia\...` | Move the project to a path without spaces or accented characters |

## Suggested reading

- Jenny Bryan's *Happy Git with R* --- <https://happygitwithr.com>. Focus on chapters 1-4 for now, particularly the mental model of a repository and the commit/diff/push cycle.

## Homework

- Add a sentence to your `README.md` describing the data source you plan to use for the mini-project.
